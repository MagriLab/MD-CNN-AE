...
class Encoder(Model):
    def __init__(self,Nx,Nu,features_layers=[1],latent_dim=16,
        ...
        for i in range(len(self.features_layers)):
            self.add_layers.append(Conv2D(self.features_layers[i],self.filter_window, padding='same',kernel_regularizer=l2(self.lmb[0]), bias_regularizer=l2(self.lmb[1]), activation=self.act_fct))
        ...
        self.dense = Dense(self.latent_dim,kernel_regularizer=l2(self.lmb[0]), bias_regularizer=l2(self.lmb[1]),activation=self.act_fct)
...
class Decoder(Model):
    def __init__(self,Nx,Nu,layer_size,features_layers=[1],latent_dim=1,
        ...
        self.add_layers.append(Dense(prelatent_size,kernel_regularizer=l2(self.lmb[0]), bias_regularizer=l2(self.lmb[1]), activation=act_fct)) # restore the number of elements in the layer before the latent space
        if self.batch_norm:
            self.add_layers.append(BatchNormalization())
        self.add_layers.append(Dropout(self.drop_rate))
        ...
        for i in range(len(self.features_layers)-1):
            ...
            self.add_layers.append(Conv2D(self.features_layers[-i-2],self.filter_window, padding='same',kernel_regularizer=l2(self.lmb[0]), bias_regularizer=l2(self.lmb[1]), activation=self.act_fct))
            if self.batch_norm:
                self.add_layers.append(BatchNormalization())
        ...
        self.add_layers.append(Conv2D(self.Nu,self.filter_window, padding='same',kernel_regularizer=l2(self.lmb[0]), bias_regularizer=l2(self.lmb[1]), activation='linear')) # last layer
